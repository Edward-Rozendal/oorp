...
Tests may be introduced at various times for various reasons. <<write-tests-to-understand>> advocates investing testing effort in those parts of the system that you need to understand in order to implement changes. More specifically, it is a good idea to <<a.1.2-test-fuzzy-features>>, to <<a.1.3-test-old-bugs>>, and especially to <<a.1.1-retest-persistent-problems>>.
